...
object CoffeeCupAtlasBuilder {
    ...
    fun build(context: Context, snapshot: CoffeeSessionSnapshot): BuildResult = synchronized(buildLock) {
        try {
            buildInternal(context, snapshot)
        } catch (e: OutOfMemoryError) {
            Log.w(TAG, "BUILD_FAILED stage=oom")
    ...
    private fun buildInternal(context: Context, snapshot: CoffeeSessionSnapshot): BuildResult {
        ...
        for (region in CoffeeCupRegion.entries) {
            ...
            val buffer = loadBuffer(photo.file) ?: continue
            buffers[index] = buffer
            present[index] = true

            val manual = snapshot.geometryFor(region)?.takeIf { it.isValid() }
            if (manual != null) {
                ...
                val geometry = estimate.geometry
                if (geometry != null && geometry.isValid()) {
                    geometries[index] = geometry
                    if (estimate.status == RimEstimate.Status.LOW_CONFIDENCE) {
        ...
        for (i in 0..2) {
            ...
            disks[i] = C.rectify(buffer, geometry, C.DISK_SIZE)
            buffers[i] = null
        }
        ...
        val fused = C.fuse(disks, alignments, C.DISK_SIZE)
        val atlas = C.renderAtlas(fused.disk, CoffeeCupAtlas.ATLAS_WIDTH, CoffeeCupAtlas.ATLAS_HEIGHT, CoffeeCupAtlas.FLOOR_V)
    ...
    private fun loadBuffer(file: File): C.PixelBuffer? {
    ...
    private fun exportDebug(
        ...
        runCatching {
            ...
            val floorCartesian = C.renderFloorCartesian(fused.disk, C.DISK_SIZE)
            writePng(File(dir, "debug_floor_reconstruction.png"), bufferToBitmap(floorCartesian, C.DISK_SIZE, C.DISK_SIZE))
            ...
            writePng(File(dir, "debug_source_weights.png"), bufferToBitmap(C.renderWeights(fused.weights, C.DISK_SIZE), C.DISK_SIZE, C.DISK_SIZE))
